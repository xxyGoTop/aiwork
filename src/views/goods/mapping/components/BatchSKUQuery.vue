<template>
<el-drawer :visible="visible" title="SKUID查询" @close="handleClose" size="400px">
  <div class="drawer">
      ...
      <el-form :model="formData" label-position="top" ref="form" :rules="rules">
        ...
        <template v-else>
          <el-form-item label="下载模板，按格式填写内容">
            <a class="link" @click="getTaskTemplate('SKUID批量查询模板', 'xlsx')">
              <Icon name="excel" class="link-icon"></Icon>
              <span>SKUID批量查询模板</span>
            </a>
          </el-form-item>
...
export default {
  ...
  data() {
    const validatePids = (_, value, callback) => {
      if (!value.match(/^[\d\n\r]\s*/)) return callback(new Error("输入ID格式不正确"))
      if (value.trim().split(/[\n\r]\s*/).length > 50) {
        ...
        if (valid) {
          ...
          if (this.activeName === "manual") {
            queryCondition.productIds = condition.productIds.split(/\n\r?/).map(lineStr => {
              const [productId, outerCode, scItemId = ""] = lineStr.split(/,\s*/)
              const ids = {
                productId,
                outerCode,
              }
              if (this.isTmallShop) {
                ids.scItemId = scItemId
              }
              return ids
            })
          } else {
