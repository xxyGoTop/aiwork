 <template>
  <el-drawer
    :title="info.ty === 'unbind' ? '映射解绑详情' : '映射绑定详情'"
    :visible="visible"
    ...
    <div class="container">
      ...
      <el-table :data="prdInfoList" border style="margin-top:15px" v-loading="loading">
        ...
        <el-table-column label="SkuID" prop="outerSkuId" align="center"></el-table-column>
        <el-table-column label="物流宝ID" prop="scItemId" align="center" v-if="isTmallShop"></el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      statusOptions: [
        // { label: '不限', value: '' },
        // { label: '映射查询成功', value: 1 },
        // { label: '映射查询失败', value: 10 },
        // { label: '映射绑定成功', value: 2 },
        // { label: '映射绑定失败', value: 20 },
      ],
    }
  },
  computed: {
  ...
  methods: {
    ...
    handleExport() {
      ...
      TaskResult({
        ...
        .then(({ data }) => {
          if (data === 3) {
            dlFile(`${this.exportLink}${this.info.id}`, `商品映射-${this.info.taskNum}`, "xlsx")
          } else {
