<template>
<el-drawer :visible="visible" title="批量映射绑定" @close="handleClose" size="750px">
  <div class="drawer">
    ...
    <div class="drawer-form">
      <el-form :model="formData" label-position="top" ref="form" :rules="rules">
        <el-form-item label="选择店铺" prop="partnerId">
          <el-select v-model="formData.partnerId" @change="formData.productMapBindType = ''">
            <el-option
        ...
        <el-form-item label="绑定类型" prop="productMapBindType" v-if="isTmallShop">
          <el-select v-model="formData.productMapBindType">
            <el-option
        ...
        <template v-if="activeName === 'manual'">
          <el-form-item label="映射绑定ID" prop="productIds">
            <template #label>
              <span>映射绑定ID</span>
              <div class="label-tip">
                多个ID请用‘换行"隔开，最多支持50品映射绑定
                （注：不支持SPU下多SKU映射绑定，不支持重复品绑定，请在系统直接发布SPU下多SKU，重复品）
              </div>
            </template>
            <el-input
              :placeholder="isTmallShop && formData.productMapBindType ? formData.productMapBindType === '1' ? '每一行一个ID，每次最多50个ID\n当当ID 天猫SPUID 天猫SKUID 物流宝ID\n23222 22222 22222 2222' : '每一行一个ID，每次最多50个ID\n当当ID 天猫SPUID 物流宝ID\n23222 22222 22222' : '请录入ID(格式：商品ID  外站ID)'"
              type="textarea"
              :rows="5"
        ...
        <template v-else>
          <el-form-item label="下载模板，按格式填写内容">
            <a class="link" @click="getTaskTemplate('商品批量映射绑定模板', 'xlsx')">
              <Icon name="excel" class="link-icon"></Icon>
              <span>商品批量映射绑定模板</span>
            </a>
          </el-form-item>
...
export default {
  ...
  data() {
    const validatePids = (_, value, callback) => {
      if (this.isTmallShop) {
        if (this.formData.productMapBindType === "1") {
    ...
    handlePublish() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.isTmallShop) queryCondition.productMapBindType = productMapBindType
          if (this.activeName === "manual") {
            queryCondition.productIds = condition.productIds.split(/\n\r?/).map(lineStr => {
              ...
              if (this.isTmallShop) {
                if (productMapBindType == 1) ids.outerSkuId = outerSkuId
                ids.scItemId = productMapBindType === "1" ? scItemId : outerSkuId
              }
              return ids
            })
          } else {
