...
<el-drawer
  ...
  <div class="drawer">
    <el-tabs v-model="activeName" @tab-click="handleTabClick" class="drawer-tabs">
      <el-tab-pane label="手动添加" name="manual" class="tab-pane"></el-tab-pane>
    ...
    <div class="drawer-form">
      <el-form :model="formData" label-position="top" ref="form" :rules="rules">
        ...
        <template v-if="activeName === 'manual'">
          <el-form-item>
            <template #label>
              <span class="required-label">添加商品</span>
            </template>
            <el-table :data="queryCondition">
              <el-table-column min-width="120">
                <template #header>
                  <span class="required-label">外平台SPUID</span>
                </template>
                <template #default="{ row }">
                  <el-form :model="row" ref="spuRef">
                    <el-form-item
                      ...
                      prop="outerCode">
                      <el-input v-model="row.outerCode"></el-input>
                    </el-form-item>
                  </el-form>
                </template>
              </el-table-column>
              <el-table-column min-width="120">
                <template #header>
                  <span class="required-label">SKUID(当当ID)</span>
                </template>
                <template #default="{ row }">
        ...
        <template v-else>
          <el-form-item label="下载模板，按格式填写内容">
            <a class="link" @click="getTaskTemplate('批量添加SKU模板', 'xlsx')">
