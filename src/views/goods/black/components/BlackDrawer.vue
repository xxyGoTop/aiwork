    ...
    <div class="drawer-form">
      <el-form :model="formData" label-position="top" ref="form" :rules="rules">
        <el-form-item label="选择店铺" prop="partnerId">
          <el-select v-model="formData.partnerId" @change="handlePartner" :disabled="!isAdd" style="width:220px">
            <el-option
        ...
        <el-form-item label="选择类型" prop="taskSubType">
          <el-select v-model="formData.taskSubType" @change="handleTaskSubTypeChange"  :disabled="!isAdd" style="width:220px">
            <el-option
              v-for="(type, ky) in taskTypeOptions"
              :label="type.label"
              :value="type.value"
              :key="ky">
            </el-option>
        ...
        <el-form-item label="开始时间" prop="startTime">
          <el-date-picker
            ...
            type="datetime"
            @change="isCurrent = false"
            placeholder="选择开始时间">
          </el-date-picker>
          <el-checkbox v-model="isCurrent" style="margin-left: 16px" @change="handleCheckedChange">此刻</el-checkbox>
        </el-form-item>
        <el-form-item
          label="结束时间"
          prop="endTime"
          ...
          <el-date-picker
            ...
            @change="isEndYear = false"
            :disabled="formData.longTern">
          </el-date-picker>
          <el-checkbox v-model="formData.longTern" style="margin-left: 16px" :disabled="disabled">长期</el-checkbox>
          <el-checkbox v-model="isEndYear" @change="handleClickYear" :disabled="formData.longTern">年底</el-checkbox>
        </el-form-item>
        <!--manual-->
        ...
        <template v-else>
          <el-form-item label="下载模板，按格式填写内容">
            <a class="link" @click="getTaskTemplate(`${subTypeMap[formData.taskSubType]}批量模板`, 'xlsx')">
              <Icon name="excel" class="link-icon"></Icon>
              <span>{{ `${subTypeMap[formData.taskSubType]}批量模板` }}</span>
            </a>
          </el-form-item>
        ...
        <el-form-item prop="stock2Zero" v-if="isAdd && formData.taskSubType === '1083'">
          <el-checkbox v-model="formData.stock2Zero">
            库存同步为零（勾选此选项后，将目前该商品库存拉低为0）
          </el-checkbox>
        </el-form-item>
      </el-form>
    </div>
